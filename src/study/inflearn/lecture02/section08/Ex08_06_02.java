...
public class Ex08_06_02 {
    public String[] solution(String[] subjects, String[] course){
        ...
        // 다음 수강해야하는 과목 인접리스트
        // { 인덱스 : 선수과목, 자료구조 : 다음 수강 과목} ex) [[], [2,3], [0] ...]
        ...
        for (String rel : course) {
            String a = rel.split(" ")[0];
            String b = rel.split(" ")[1];

            Integer aIdx = subIdxMap.get(a);
            Integer bIdx = subIdxMap.get(b);
        ...
        while (!q.isEmpty()) {
            ...
            for (Integer nextSub : arr.get(cur)) {
                if (indegree[nextSub] !=0 ) indegree[nextSub] = indegree[nextSub] -1;
        ...
        for (Integer num : order) {
            for (String s : subIdxMap.keySet()) {
                if (num == subIdxMap.get(s)) {
                    answer[cnt] = s;
                    cnt++;
                }
            }
