...
public class Ex03_05_Answer {
    public int[] solution(int[][] tasks){
        ...
        for (int i = 0; i < n; i++) {
            taskInfo.add(new int[]{tasks[i][0], tasks[i][1], i}); //{호출시간, 실행시간, 작업번호}
        }
        // 호출시간 기준 오름차순
        taskInfo.sort((a,b) -> a[0] - b[0]);
        //taskInfo.forEach((i) -> System.out.print(Arrays.toString(i))); System.out.println();

        // 대기공간 생성 - 우선순위가 높은 것(요소값 작은)부터 꺼내는 우선순위 큐 선택
        PriorityQueue<int[]> waitQ = new PriorityQueue<>((a,b) -> a[0] == b[0]? a[1] - b[1] : a[0] - b[0]); // 호출시간 오름차순을 기본으로 호출시간이 같을 때 실행시간 오름차순
        int fT=0, idx = 0;
    ...
    public static void main(String[] args){
        ...
        System.out.println(Arrays.toString(T.solution(new int[][]{{1, 1}, {2, 3}, {1, 1}, {3, 3}, {8, 2}, {1, 1}, {2, 2}, {1, 1}})));
