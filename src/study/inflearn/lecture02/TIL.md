...
- 주어진 입력 데이터(dp인덱스)부터 쭉 끝까지 돌면 어떤 사태가 일어나는 지 확인해보면 dp[i-idx] + c라면 무조건 주어진 데이터는 사용한다는 의미로 된다. (i가 5일때)
- 입력 데이터 1번으로 i를 증가해서 N까지 풀다보면 어느 순간 2번 사용하게 되는 시점이 있다.
- 예를 들어 i가 10일때 dp[10-5] + c로 계산이 되는데, 이미 앞에서 1개를 사용했는데 또 1개를 더 사용한다는 의미가 되버림. 2번 사용한다는 효과를 준다. 중복을 해서 풀어버린다.
- 그러면 중복을 적용하지 않게 하기 위해 뒤에서 돌아야지 중복을 회피합니다. 즉, 1번만 사용하게 됨.
- 점화식에서 무조건 i에서 입력 데이터를 빼서 앞에만 보기 때문에 뒤부터 탐색을 하면 앞에는 0으로 초기화가 되어 있어 1번만 적용되게 할 수 있다.
- 냅색 알고리즘 이라고 할때 종류가 무한개라면 앞에서 탐색하고, 개수가 정해져있거나 1개로 주어진다 싶으면 뒤부터 탐색해야한다.
  ![img.png](img/ex09_02_Answer.png)

## Ex09_06
### 문제 포인트
- 백준 수확 1823번
- 벼의 가치가 주어지면 얻을 수 있는 최대 이익을 출력합니다.
- 단, 양 끝에 있는 벼만 수확할 수 있습니다.
- 가치의 값은 v(i) x K(순서)만큼 이익을 얻음

### 회고
- 문제를 보려고 해법영상을 틀었는데 2차원 배열로 푸는 걸 봐버렸음.
- 2차원 배열로 풀어보려고 했으나, 어떤 순서로 데이터를 탐색해야 하는 지 감이 안왔음.
- 배열의 인덱스 12,123,1234,12345로 탐색하면서 123일 때 12|3, 1|23 이런식으로 구하면 되지 않을까 싶었다.
- 근데 23의 가치값을 구하지 않은 상태여서 123의 가치를 구할 수 없기때문에 팰린드롬과 동일하게 작은 경우의 수부터 다이나믹 값을 저장시켜야한다는 걸 배웠다.
- 12|23|34|45, 123|234|345, 1234|2345, 12345 이런식으로 작은 문제부터 점진적으로 풀어나가는,, 알고는 있었지만 뭔가 자꾸 한번에 풀려고 한다. 
- 다이나믹을 풀 때 작은 문제를 큰 문제로 풀 수 있는 지 항상 확인해보려고 하자.
- 그리고 양 끝에 있는 벼만 수확할 수 있어 순서가 바뀌는 경우의 수와 순서증감을 어떻게 소스로 구현해야하나 싶었다.
- 예를 들어 315 벼의 가치가 주어질때 15와 31의 다이나믹 값 중 최대값에서 벼의 가치 모두를 더하면 수확 순서를 증가시켜준다는 걸 배웠다.
- 15의 다이나믹 값(1x1 + 2x5) + (3+1+5)의 효과는 1x3 + 2x1 + 3+5로 K+1번째 증가시켜주게 된다.

### 해결
- 강사님 코드 해설
  - dp[i][j] 다이나믹 테이블의 정의 : i번째 벼부터 j번째 벼까지를 수확했을 때 얻을 수 있는 최대 이익
  - 팰린드롬과 동일하게 반복문을 돌려야한다.
  - dp 테이블 접근 할때 dp[j][j+i]로 j가 행이 되어 탐색되어야 한다.
  - 다이나믹 값에서 탐색하는 벼들의 수를 한번 더해준다는 의미는 순서를 유지하면서 번째를 증가해준다는 효과를 준다. 
  - 수를 한번 더해줄때 누적의 값을 배열로 미리 잡아서 계산해준다.
  ```java
      // 벼의 가치 누적합
...
### 문제 포인트
- 백준 팰린드롬 만들기 1695번
- 수열이 주어졌을 때, 수열에 수를 끼워넣어 팰린드롬을 만듭니다. 끼워넣는 최소 개수를 찾아내는 문제
- 다이나믹 프로그래밍 바템업, 메모이제이션 2차원 배열 사용 (필요한 경우의 수만 저장)

### 회고
- 다이나믹으로 안풀고 수열을 해싱해서 개수 토대로 풀었으나, 백준에서는 틀림.
- 강사님 해법 강의 듣고 재도전 했으나.. 이중 반복문 + 점화식 i,j 조건식 틀림
- 다이나믹은 어렵다.. 이거 풀면서 많은 생각이 들었다. 너무 많은 고민을 했다.
- 예를 들면 이렇게 어려운 문제들이 나오면 어떻게 풀어야 할까.. 코드해설 이해가 안가는 문제구나.. 이해하기 위한 분석시간 오래 걸리겠다. 생각이 드니깐 잡생각이 많이 들었다.
- 이중 반복문에 대한 초기화, 조건식을 이해하기 위해서 직접 디버깅을 하면서 왜 이렇게 짰는 지 이해를 해야 코드가 이해된다. 
- 그래서 문제 풀어보면서 직접 손으로 끄적여보면서 고민하고, 해설보고 디버깅도 하면서 여러문제들을 많이 풀어보라는 말이 이해가 됬다.
- 다이나믹 테이블 정의가 생각이 안나 당연히 메모이제이션 2차원 배열 생각도 못했다.
- 메모이제이션 2차원 배열의 모든 경우의 수를 계산하지 않고 문제 해결에 필요한 점화식만 계산하면 된다는 것을 배웠다.
...
- 강사님 코드 해설
  ...
  - dy[][] 다이나믹 테이블 정의 : i번째부터 j번째까지 부분수열을 팰린드롬을 만들기 위해 끼워넣어야할 수의 최소 개수
  - 수열의 길이 2~N까지 중에 팰린드롬을 만들기 위한 끼워넣는 최소 개수를 구하는 것이다.
  - 수열의 길이에 따른 수열의 조합을 통해 조합된 경우의 수만 dp[j][j+i]에 계산하면 된다.

  ![img.png](img/ex09_05_Answer.png)

## Ex09_04
### 문제 포인트
- 백준 등차수열 1994번
- 가장 긴 등차수열의 길이 구하기
- 다이나믹 프로그래밍 바템업, 메모이제이션 2차원 배열 사용

### 회고
- 1시간 정도 고민하면서 풀어봤으나 틀렸다.
- 들어온 입력값을 오름차순 정렬하는 건 맞췄다. 
- dp 배열을 정의하고 아이디어가 떠올랐으나 명확한 점화식이 안떠올랐고, 구현해보니 틀리게 나왔다.
- i는 0~가장 큰 공차, dp[i]는 등차수열의 최대개수로 정의했다.
- 강사님 강의를 듣고 코드를 수정해보았다.
- 답은 맞췄는데 백준에서 메모리 초과가 계속 떴다. 
- 정수의 범위가 1,000,000,000 무려 10억이기 때문에 이중반복문으로 탐색하면 0(n^2)이므로 메모리 초과가 안뜨는 게 비정상

### 해결
- 강사님 코드해설
  - 일정한 값이(공차) 양수/음수/0 +2/-2/0 모두 적용된다.
  - 들어온 입력값을 오름차순 정렬해서 공차가 양수인 것만 찾으면 된다.
  - nums 1~N까지 N+1 사이즈로 생성. nums[0] = 0으로 초기화 
  - 다이나믹 바템업으로 진행.
  - 다이나믹 테이블 정의 : dy[i][j] i번째와 j번째가 마지막 두 항인 등차 수열의 최대 길이 (i<j)
  - nums[j] - nums[i] : 공차
  - ✨ 등차수열의 마지막 두 항(i항과 j항)의/i번째 앞에 항을 찾아 해당 항의 인덱스를 순차탐색 or 이분검색으로 찾아낸 후 등차수열의 길이를 합산하여 기존값(dy[i][j])과 변경될 값의 최대값으로 교체한다.
  - 이 문제는 항의 인덱스를 구하는 로직에서 이분검색의 변형이 포인트이다. 어렵다..
  - 예를 들어 2223346 등차수열 중에서 246 수열을 찾을 때 222에서 가장 마지막 인덱스를 가져와야 하므로 left/right 초기화를 잘해줘야한다. 

## Ex09_03
### 문제 포인트
- 백준 계단 오르기 2579번
- 앞에서 작은 크기의 문제의 해를 이용해서 그 다음 크기의 문제 해를 구하는 다이나믹 기법을 사용해서 풀어야 한다.
- 3,00 * 10,000 = 3,000,000 (int 범위 내에 있음을 확인)
- dp[i] = 시작지점부터 올라가서 계단규칙을 이용해서 i번째 계단까지 올랐을 때 얻을 수 있는 최대 점수

### 동작원리
- 점화식 : 세번째 계단부터 적용
- 제약사항 : 연속 세 개 계단 못밟음, 도착 계단 무조건 밟아야함
    ```
    - (불가능) dp[i-1] + score[i] : 이전 계단 + 한 계단 오르는 경우는 연속으로 오른 경우 + 한 계단으로 세 계단 오르는 경우가 포함되서 제외
    - (불가능) dp[i-2] + score[i-1] + score[i] : 전전 계단 + 한 계단 + 한계단 오르는 경우로 세 계단 오르는 경우가 포함되서 제외
    - (가능) dp[i-2] + score[i] : 전전 계단 + 두 계단 오르는 경우는 가능. (전전 계단에서 한 계단 올랐든, 연속 올랐든 세 계단 초과되서 두 계단으로만 오름 가능)
    - (불가능) dp[i-3] + score[i-2] + score[i-1] + score[i] : 전전전 계단 + 한계단 + 한계단 + 한계단 오르는 경우는 세 계단 오르는 경우 포함되서 제외
    - (가능) dp[i-3] + score[i-1] + score[i] : 전전전 계단 + 두 계단 + 한계단 오르는 경우는 제약사항에 안걸리므로 가능.
    ```
    ![img.png](img/ex09_03_Answer.png)

### 잘못생각한 점
- 다이나믹 테이블 인덱스 0~5까지 배열의 크기 6으로 생성해야겠다 생각 했음.
  - ✨다이나믹 테이블 생성할 때 N개의 최대점수/최소점수 라고 문제에서 주어지면 배열의 크기 N+1까지 잡아야함.
- 다만 배열의 요소에 어떤 값이 들어와야하는 지, 점수라면 어떻게 얻을 수 있는 점수를 구해야하는 지 감이 안왔음.
  - 시작지점에서 두단계 밟고 올라가는 건 생각도 못함. 


## Ex09_02
### 문제 포인트
- 백준 수도배관공사 2073번
- 수도관 길이(7 ≤ D ≤ 100,000)에 맞춰 그것을 이루는 여러개의 파이프(1 ≤ P ≤ 350)와 각 파이프 길이(L), 용량(C)이 주어졌을 때, 수도관의 용량은 그것을 이루는 파이프들의 용량 중 최소값되고, 수도관 한개를 정확하게 D와 같게 하여 가능한 최대 수도관 용량을 구하는 문제
- ✨보통 기업 코테에서 P의 개수가 350개나 된다. 개수가 15~20개면 DFS 완전탐색으로 모든 경우의 수 탐색해야한다.
- ✨요즘 좋은 기업들은 다이나믹을 내는 추세로, 350개를 완전 탐색으로 풀면 Time Limit 나기 때문에 냅색 알고리즘으로 풀어야 한다.

### 동작원리
- P개의 파이프를 가지고 수도관의 길이 D와 같게 할 때, 가능한 최대 수도관 용량
  - dp[i] 다이나믹 테이블 정의
  - i길이의 수도관을 만들 때 최대 용량
  - ✨다이나믹 테이블의 정의 정확하게 잡고 가기!
- ✨파이프를 사용할 때 1번만 사용할 수 있기 때문에, 무한정 있지 않고 딱 1개만 사용 가능하면 중복을 적용하면 안되므로 뒤에서 부터 탐색 해야한다.
  - ✨09_01 사탕가게 : 각 사탕의 개수는 매우 많기 때문에, 원하는 만큼 사탕을 구매할 수 있다. (무한-> 앞부터 탐색)
  - ✨09_02수도배관공사 : 수도관 길이 합이 D가 되게 하는 수도관의 부분집합이 적어도 하나 존재한다. (유한-> 뒤부터 탐색)
- 주어진 파이프 P개 만큼 끝부터 파이프 길이까지 dp배열을 탐색하며, 테스트 케이스 첫번째 파이프부터 아래 계산을 반복
    ```
    1. 1개의 파이프로 수도관을 만들 수 있는 경우
        - 수도관의 길이 j가 테스트 케이스 파이프 길이와 딱 맞을 경우
        - 기존의 j번째 수도관 길이의 용량(dp[j])과 테스트 케이스 파이프 용량(C) 중 최대값으로 갱신
    2. 수도관 길이에 맞춰서 만들 수 없는 경우
        - 수도관 길이(j) - 테스트 케이스의 파이프 길이(L) = 나머지 파이프(dp[j-L])
        - 나머지 파이프의 용량이 0인 경우는 수도관을 만들 수 없으므로 continue
    3. 최대 수도관의 용량 구하기
        - 변경할 수도관은 수도관 길이만큼 이루어진 파이프들 중 최소값
        - 기존의 수도관과 변경할 수도관 중 최대 용량 선택
        ex) 수도관 길이 7를 만들때 
           경우의 수 1 : 파이프 길이 5 (용량 4) + 2 (용량 5) -> 최소값 4
           경우의 수 2 : 파이프 길이 4 (용량 7) + 3 (용량 1) -> 최소값 1
           ✨수도관 길이 7을 만드는 파이프들의 조합 중 가능한 최대의 용량 경우의 수 1을 선택 
    4. (1)~(3)반복하며 dp 배열 탐색
        - ✨다이나믹 테이블은 파이프의 종류를 1번씩만 사용해서 적용한 것
    ```     

## Ex09_01
### 문제 포인트
- 백준 사탕가게 4781번
- 사탕 가게에 있는 모든 사탕의 가격과 칼로리가 주어졌을 때, 상근이가 가진 돈으로 구매할 수 있는 가장 높은 칼로리를 출력한다.
- 사탕 가게는 다이나믹 프로그래밍 냅색 알고리즘의 전형적인 문제
  - ✨다이나믹 프로그래밍은 작은 문제부터 서서히 확대해서 최종적인 답으로 가는 것
  - ✨냅색 알고리즘은 가방문제가 유명해서 가방 알고리즘이라고 해서 그냥 냅색 알고리즘이라고 부르는 것임
  - ✨같이 풀어보면 좋은 문제 : 백준 동전1, 동전2, 평범한 배낭

### 동작 원리
- 상근이가 m원을 가지고 사탕을 살 수 있는 최대 칼로리
  - dp[i] 다이나믹 테이블 정의
    - i원을 가지고 사탕을 살 수 있는 최대 칼로리
    - ✨값의 정의를 잘 내리고 문제를 풀어야 함
  - 1번 테스트 케이스의 각 사탕의 칼로리와 가격이 주어지면
    - 700칼로리 7.00원 (배열의 인덱스가 돈이 될꺼기 때문에 실수형 7.00원 -> 정수형 700 형변환)
    - 199칼로리 2.00원 (배열의 인덱스가 돈이 될꺼기 때문에 2.00원 -> 정수형 200 형변환)
    - 상근이 돈 m, 사탕 가격 p 둘다 형변환 해야함
    
- 주어진 사탕종류 수만큼 사탕가격부터 끝까지 dp배열을 탐색하며, 테스트 케이스 첫번째 사탕가격부터 아래 계산을 반복
    ```
    1. dp[i-가격] + 칼로리
        - i-가격 + 칼로리를 하는 이유?
        - ✨주어진 사탕종류는 일단 하나는 무조건 산다는 가정이고 +칼로리,
        - ✨하나를 샀으니 현재 돈(i)에서 주어진 사탕가격(가격)을 뺀 남은 돈 dp[i-가격]의 칼로리
        - 합한 가격이 최대 칼로리를 의미함
    2. (1)의 계산결과가 dp[i] 현재의 값보다 크다면 값 갱신
        - ✨같은 돈 주고 더 높은 칼로리를 얻는 게 문제에서 요구사항
    3. (1)~(2)반복하며 dp 배열의 끝까지 탐색
        - ✨다이나믹 테이블은 사탕 종류를 모두 혼합해서 다 적용한 것.
    ```     
...
for (int i = price; i < dp.length; i++) {
    // 수정전
    int idx = i - price;
    if (idx == 0) pibo = dp[idx] + cal;
    else pibo = dp[idx] + dp[i-1];
...
```
- Ex09_01 Local History 보면서 설명하기
- 피보나치로 생각하고 풀었다...
...
### 문제 포인트
- 교육과정
- 선수과목 정보가 주어질 때 n개의 과목을 모두 이수하는 순서대로 출력하시오. (여러답 중 아무거나 반환)
- ["art math"] 의미는 art 과목을 수강하기 전에 math 과목을 선수로 수강해야합니다.
- 위상정렬 알고리즘으로 풀어야함.
  - 선후관계로 전체적인 순서를 나열할 때 위상 정렬을 사용해야 합니다. 
  - 위상 정렬(Topological sort)은 비순환 방향 그래프에서 정점을 선형으로 정렬하는 것입니다. 
  - 주로 선후 관계가 있는 일련의 작업을 차례대로 수행하기 위해 순서를 정할 때 사용하는 알고리즘 입니다.
    - 비순환 방향 그래프 이기 때문에 사이클이 존재하면 안됨
- 차수 : 노드와 연결되어 있는 간선의 개수
- 진입차수 : 해당 노드로 들어오는 간선의 개수

### 동작원리
![img.png](img/ex08_06_Answer.png)

- 진입차수 배열 감소시키는 이유?
    - 진입차수 배열의 정의는 해당 해당과목의 선수과목의 개수 입니다.
    - 큐에 들어있는 과목을 꺼내와서 인접리스트 해당 과목의 다음 수강 과목들의 입장에서는 선수과목이 하나 줄어들었기 때문에 큐에서 꺼내오고 연결된 다음선수의 진입차수를 감소시켜야합니다.
    - 내가 수강한 과목(큐에서 꺼냄)의 수강해야할 다음 과목(인접리스트 값들)에서 선수과목 1개 줄여주는 것.
    - 어떤 과목의 선수과목 입장으로 생각해야함.
      - 큐에서 꺼낸 과목을 수강했으니 큐에서 꺼낸 과목이 선수과목에 속한다면 감소 시켜줘야함. 

### 잘못 생각한 점
- Integer[] integer = new Integer[]; 래퍼클래스(참조타입) 배열은 null로 초기화 됨.
- 기본타입 배열은 기본값으로 초기화 됨.

## Ex08_05
### 문제 포인트
- 공 굴리기
- 공이 시작위치에서 목표위치까지 이동하는 최단거리 (이동한 격자의 수)
- 상하좌우 움직이며 벽(1)이나 격자 밖으로 나가면 멈춤
- 다익스트라로 해결해야함.
- 한칸씩 이동하는 게 아니라 벽을 만날 때 멈춰야 하므로 한칸씩 다익스트라로 계산 하는 게 아니라 멈추는 지점에 이동 수를 계산해야하는 게 포인트

### 정리
- 간선 값이 일정하지 않고 다양하면 다익스트라로 해결 (거리값이 양수인 경우)
- 간선 값이 일정하지 않으면 BFS로 해결해야함.


### 잘못 생각한 점
- 다익스트라로 푸는 게 맞는 지 확신이 없었으나, 다익스트라로 구현해본 후 답이 틀리게 나와서 BFS로도 풀어봤으나 틀림
- 공이 이동하고 있는 칸에도 이동한 격자 수를 초기화 해줘야한다는 생각을 했음.
- 행열이라 한칸 한칸 노드라고 생각했는데, 조건에 벽을 만나면 멈춘다. 어떤 지점에서 벽까지를 간선으로 봐야하는 걸 몰랐음.
...
- 다익스트라로 해결해야함.
- 우선순위 큐가 비어질때가지 모든 개수를 구하지 않고, cur[] 꺼낼 때 마지막 지점일 경우 return 해주면 조금이라도 시간 복잡도 줄일 수 있다.
...
```
- 다익스트라 알고리즘에서 중요한 1,2/3 로직
- 1. 우선순위 큐에서 꺼낸 현재 지점의(출발정점에서 해당 지점까지) 비용이 기존의 비용보다 크다면 굳이 하위레벨 경우의 수를 볼 필요가 없다. 벽을 부수는 최소 비용의 답을 찾아야 하므로, 최소값을 교체해줘야한다.
